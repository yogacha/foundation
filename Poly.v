...
(* seems that union type will violate these type inference, ex: Number = Nat | Int | ...
  wonder if there is union type in coq ? *)
...
Fixpoint rev {X:Type} (l:list X) : list X :=
  match l with
  | nil      => nil
  | cons h t => app (rev t) (cons h nil)  (* ugly ....*)
  end.
...
(** **** Exercise: 3 stars, standard (map_rev)
    ...
    auxiliary lemma. *)
Lemma map_cat : forall (X Y : Type) (f: X -> Y) (l : list X) (x: X),
...
*)
(* Definition option_map' {X Y: Type} (f : X -> Y) ():= @@ *)
(** [] *)
...
Fixpoint fold {X Y: Type} (f : X->Y->Y) (l : list X) (b : Y)
  ...
  match l with
  | nil => b
  | h :: t => f h (fold f t b)  (* @@ why from last *)
  end.
...
Theorem fold_map_correct: forall X Y l (f: X -> Y),
...
(* Definition extract_f (n: forall X : Type, (X -> X) -> X -> X) :=
  for. *)
...
(* Definition repp (h: cnat->cnat->cnat) (n m: cnat): cnat:=
  m cnat (h n) n. *)

(* Definition eexp (n m: cnat) : cnat :=
  repp exp n m. *)
