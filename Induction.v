...
Theorem add_shuffle3 : forall n m p : nat,
  n + (m + p) = m + (n + p).
Proof.
  intros. rewrite -> add_assoc, add_assoc.
...
Fact dist_1 : forall n k: nat,
  n * S k = n + n * k.
Proof.
  induction n as [|n' IHn'].
  - reflexivity.
  - intros k. simpl. rewrite IHn', add_assoc, add_assoc.
    assert (H: k + n' = n' + k). { rewrite add_comm. reflexivity. }
    rewrite H. reflexivity.
    Qed. 
...
Proof.
  ...
  - simpl. rewrite <- IHn'. rewrite dist_1. reflexivity. Qed.  
(** [] *)
...
Theorem all3_spec : forall b c : bool,
  ...
  = true.
Proof.
  intros [] []. reflexivity. reflexivity.  reflexivity. reflexivity. Qed.
...
Proof.
  ...
  - intros m p. simpl. rewrite IHn', mult_plus_distr_r. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. rewrite -> IHx, add_comm. simpl. rewrite add_comm. reflexivity. 
Qed.
...
Proof.
  induction n as [|x].
  - simpl. reflexivity.
  - simpl. rewrite bin_to_nat_pres_incr, IHx. reflexivity. Qed.
...
Fixpoint normalize (b:bin) : bin :=
  match b with
  | Z => Z
  | B0 x => match (b0star x) with
            | true => Z
            | false => B0 (normalize x)
            end
  | B1 x => match (b0star x) with
            | true => B1 Z
            | false => B1 (normalize x)
            end
  end.
